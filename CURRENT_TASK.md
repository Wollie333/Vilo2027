...
**▶ DONE since this anchor (2026-06-22, late+):**
- **QUOTE FORM (was NEXT #1) — CLOSED by code review** (founder OK'd skipping the browser drive). Read `QuoteForm.tsx` + `priceQuoteAction` end-to-end: the binding `priceQuoteAction→computeStayPricing→setBaseAmount/setPricedRooms→totals memo→amount fields` is correct in all 3 modes; send buttons gated only by `busy||hasDateConflict` (total≤0 is a `validate()` toast, not a stuck disabled). **No UI binding bug** — the real defect was the silent R0, already fixed by the amber banner.
- **EMAIL + PUSH WORKER DOUBLE-SEND (was flagged #2) — FIXED & pushed.** Migration `20260622010000` (+`claimed_at` col on `notification_queue`/`pending_push_queue` + `claim_email_queue_batch`/`claim_push_queue_batch` SECURITY DEFINER RPCs using `FOR UPDATE SKIP LOCKED` + 300s stale-reclaim). `drain.ts`+`push-queue.ts` now claim atomically before sending. Verified with `scripts/smoke-queue-claim.mjs` against the cloud DB (disjoint concurrent claims ✅, stamping ✅, sent-row exclusion ✅, stale reclaim ✅). Types regenerated; tsc+lint+73 vitest green.
- **DIGEST DRAIN RACE — FIXED.** `runDigestDrain` reordered to claim-first (conditional `sent_at` flip + `.select()`, skip if 0 rows claimed) so overlapping hourly ticks can't double-send. No migration.
- **convertQuoteAction OVER-CREDIT (was flagged #2) — FIXED.** Adopt path now completes ONLY the deposit row (`.eq("kind","deposit")`), not every pending payment — balance stays owed. Mirrors the fresh-convert path.
- **report-scheduler EDGE-FN AUTH (was flagged #2) — FIXED + DEPLOYED + verified live.** Fail-closed `x-report-scheduler-secret` gate (migration `20260622020000` reschedules the cron to send it from `app.report_scheduler_secret`). Function deployed (`--use-api`); `REPORT_SCHEDULER_SECRET` function-secret set. **Verified live:** no/ wrong secret → 401; correct secret → 200 "No reports due". **⚠️ REMAINING FOUNDER LAUNCH STEP (privileged — Management-API role got `42501` on ALTER DATABASE):** in the Supabase SQL Editor set `app.report_scheduler_secret` to match `REPORT_SCHEDULER_SECRET` (reset both to a chosen value since the current function-secret value isn't retrievable), **plus** `app.settings.supabase_url` + `app.settings.supabase_anon_key` so the hourly cron actually fires (all unset today → cron currently skips). Reporting itself is still a placeholder (TODO report generation).
- **track-listing-view EDGE-FN HARDENING (was flagged #2) — FIXED + DEPLOYED + verified live.** Public beacon kept open but validates property_id (uuid→400), device (enum), country (ISO-2), duration (clamp), referrer (len), session_id (uuid). Deployed (`--use-api`). **Verified live:** bad id → 400; real id with junk `device:"hacker"`/`country:"ZZZZ"`/`duration:99999999` → 200, row stored sanitized (`desktop`/`ZA`/`86400`). Test row cleaned up.
- **Confirmed already-safe (no change):** Paystack webhook idempotency (UNIQUE `provider_reference` + status guard) + overpayment-credit dedup (`postOverpaymentCredit` incremental). Webhook **amount-verification** still TODO — needs live Paystack.

**▶ NEXT (start here):**
1. ~~**QUOTE FORM**~~ — **DONE (see above).** ~~likely "only a UI issue" (founder's hunch). The pricing ENGINE is verified correct~~ (drove the form: auto-priced R5 950 for 2 nights, R16 650 for 6 — `computeStayPricing` is right). The form previously SWALLOWED pricing failures → silent R0 + blocked send; now surfaces an amber reason. **To close:** open the quote form on the fixture, pick listing+dates, read the amber banner. If it names a config cause (no nightly rate / min-nights / pick-a-room) → not a code bug. **If the price CALCULATES but doesn't DISPLAY in the amount field / send stays disabled despite a non-zero total → that's the UI binding bug the founder suspects — reproduce + fix in `app/[locale]/dashboard/quotes/QuoteForm.tsx` (check `totals` memo ↔ `baseAmount`/`pricedRooms` state ↔ the send-button `disabled` + the total≤0 submit guard ~line 761).**
2. **Flagged items — REMAINING (need LIVE KEYS, do NOT patch blind):** card/webhook **amount-verification** (verify `event.data.amount` == expected `payment.amount`*100 — untestable w/o Paystack keys); webhook host-key signature model review. ~~overpayment-credit dedup~~ (confirmed already correct), ~~convertQuoteAction over-credit~~ (FIXED), ~~email/push double-send~~ (FIXED), ~~report-scheduler/track-listing-view edge-fn auth~~ (FIXED + DEPLOYED + verified live). **Only launch step left for report-scheduler:** founder sets the matching DB GUC + url/anon in the SQL Editor (see DONE list) — security gate is already live/fail-closed.
