...
- **report-scheduler EDGE-FN AUTH (was flagged #2) — FIXED in source + migration `20260622020000`.** Fail-closed `x-report-scheduler-secret` gate; cron reschedule sends it from `app.report_scheduler_secret` (skips while unset). **⚠️ NEEDS FOUNDER OPS:** set `app.report_scheduler_secret` + function-env `REPORT_SCHEDULER_SECRET` + `supabase functions deploy report-scheduler`.
- **track-listing-view EDGE-FN HARDENING (was flagged #2) — FIXED in source.** Public beacon kept open but validates property_id (uuid), device (enum), country (ISO-2), duration (clamp), referrer (len), session_id (uuid). **⚠️ NEEDS FOUNDER OPS:** `supabase functions deploy track-listing-view`.
