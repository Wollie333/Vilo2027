...
**▶ Phase 6 — SAVE-POINT (b) (NEXT): booking funnel sections.** `booking_search`/`availability_calendar`/`rate_table` section types wired to the live engine with **server-recalculated** pricing (NEVER trust client prices — recalc in Edge Functions / server). **(c) after:** on-site checkout funnel + thank-you + deep engine integration (Paystack/PayPal/accounting proxied to the existing engine). NO AI.
