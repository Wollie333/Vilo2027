...
**▶ ACTIVE EPIC (· 2026-06-26): UX feedback + perf, then Safari-in-builder.** Founder directive, in order: (1) app-wide click-feedback so no action ever feels dead + speed the app up [PHASE 1 — DONE this session, see below]; (2) make the pixel-perfect Safari theme EDITABLE in the page builder — pull each Safari page template into the builder so the host changes content/images + pulls real rooms into slots, WITHOUT falling back to the generic design [PHASE 2 — NOT STARTED]. Decisions locked: feedback = TIERED (instant top bar on every nav + labeled busy-modal for slow mutations); Safari Phase 2 scope = ALL 7 editable pages (Home/Rooms/Room-detail/Rates/About/Contact/Journal) in one push, recommended approach = HYBRID (make builder canvas Safari-aware for WYSIWYG + bind bespoke Safari pages to sections + real rooms). Three grounding investigations were run (feedback map, perf audit, Safari↔builder architecture) — key findings in CHANGELOG + below.

**PHASE 1 DONE (2026-06-26) — app-wide feedback + perf quick-wins.** New rule in RULES.md §4 "Every action gives immediate feedback" (tiered). Built: global top bar (`nextjs-toploader` in root layout, brand green), labeled busy overlay (`components/ui/busy-host.tsx` — `busy.during({title,message},fn)` + `<BusyHost>`, mirrors `modal-host` store), `<PendingLink>` (`components/ui/pending-link.tsx` — heavy-route nav shows "Opening the editor…" overlay, i18n router, 550ms min), builder route skeleton (`website-editor/[websiteId]/loading.tsx`). Wired: Pages-manager "Edit page" → PendingLink; Publish → busy.during. Perf: `next.config` optimizePackageImports + images.remotePatterns + staleTimes(120/300); `hostHasFeature` wrapped in React `cache()`. tsc+lint green; config verified live (200 + toploader in HTML). **Apply the rule to MORE surfaces next** (Delete website/page, media upload, theme apply, blog/form editor opens, new-website) — primitives are ready, just wire `busy.during`/`PendingLink`. DEFERRED perf: `<img>`→next/image (esp. Safari — Phase 2 reworks those), next/dynamic for recharts + tiptap. **Interactive QA of logged-in builder surfaces still pending** (founder's dev server holds :3000; couldn't drive a preview browser without clobbering it).

**PHASE 2 — Safari editable in the builder. HOME PAGE FULLY DONE (2026-06-26).** Builder canvas Safari-aware + home bands editable (eyebrow/images/sub-text per band — see SectionEditor + sections.schema additive props) + PUBLIC home unified onto the same components (`SafariSectionList`; `SafariSiteView` takes live `data`/`asset`/ctx; deleted hardcoded `SafariHomeContent`). live === builder, real Olive Grove rooms verified in preview. **NEXT (continue here): the OTHER Safari pages** — Rooms, Room-detail, Rates, About, Contact, Journal still render hardcoded `Safari*Content` on the PUBLIC site (builder partially renders them via the home band types). Need: (a) Safari band variants for their section types not yet mapped — `amenities`, `pricing`, `contact_form`, `faq`, `rate_table`, `room_*` (room detail); (b) switch their public renders (SafariSiteView cases + the blog/room/book routes) to `SafariSectionList`; (c) editability fields for any newly-exposed content. THEN the styling audit (hide redundant controls per theme/template e.g. hero overlay-colour which Safari ignores; fix broken ones). Editable-band fallbacks/stock live in `components/site/sections/SafariSections.tsx`.

**(earlier) SLICE 1 DONE (2026-06-26): the canvas keystone.** New `components/site/sections/SafariSections.tsx` extracts each NenGama home band (hero/intro/highlights/rooms_preview/gallery/reviews/location/cta) into per-section, host-editable components driven by the existing flat `sections`. Suites bind to the host's REAL rooms; reviews to real review data. `SectionRenderer`/`SectionSwitch` got a `themeVariant`(+`safariCtx`) thread → when `"safari"`, dispatch to the Safari bands via `renderSafariSection` (fallback to generic). `PageBuilder` canvas branches: theme=safari → render inside scoped `.vilo-safari` (+ theme fonts), each block Safari-styled + selectable; else generic chrome. `safari.css` imported in the editor bundle (scoped). VERIFIED live: applied Safari "Home" template → builder shows full NenGama design w/ real Olive Grove rooms (Olive Room R1 300 / Vineyard R1 900 / Mountain Loft R2 100); Hero inspector edits content. Non-safari themes untouched. tsc+lint green. **NEXT — SLICE 2: unify the PUBLIC Safari home onto these same components** (thread room/gallery/review `data` into `SafariSiteView` so live === builder incl. real rooms — currently public still uses the old hardcoded `SafariHomeContent` with STOCK suites, so builder shows real rooms but published shows stock = a known divergence to close). THEN the other Safari pages (Rooms/Room-detail/Rates/About/Contact/Journal) + a Safari palette to add bands directly.

**PHASE 2 ORIGINAL MAP (for reference) — Safari editable in the builder.** Architecture map (from investigation): the sections model, JSONB storage (`website_pages.draft_sections`/`published_sections`), theme preset/template set (`themeSections.ts` — Safari already has presets+templates), and room loaders ALL already run for Safari sites. The blocker is purely (a) the bespoke Safari page components ignore the host's `sections`/`data`/`room` and render hardcoded JSX + Unsplash URLs (only `SafariHomeContent` partially reads section props; the other 9 take NO props), and (b) the builder canvas (`PageBuilder.tsx`) never renders Safari — it always mounts `SiteChrome`+`SectionRenderer`, so the host edits a generic preview while the live site shows NenGama. Branch points: `SitePageView.tsx:105` + `SiteRoomView.tsx:73` (`activeThemeSlug==="safari"`). Plan: 1) make `PageBuilder` canvas Safari-aware (mount the Safari layer when theme=safari) for true WYSIWYG; 2) feed each bespoke Safari page from sections + add `image_path` props w/ `asset` resolution; 3) bind suites grid + room detail to REAL rooms (reuse `RoomsPreviewData`/`loadRoomDetail` shapes the loaders already produce). Editable-critical components: `SafariHomeContent` (in SafariSiteView.tsx), `SafariRoomsContent`, `SafariRoomContent`, `SafariRatesContent`, `SafariAboutContent`, `SafariContactContent`, `SafariJournalContent`. Checkout/ThankYou/Generic stay engine/catch-all.
