...
## ▶▶ SAVE POINT — RESUME HERE (· 2026-06-27 — Safari reference COMPLETE; working a multi-epic founder backlog, header layouts DONE)

**Branch:** `main` — tsc + lint + **131 vitest** GREEN. **Verify with `cd apps/web && pnpm exec tsc --noEmit` + `pnpm next lint` + the Preview MCP — NEVER `pnpm build` while a dev server runs** (shared `.next`; corrupts on big-refactor HMR → stop server, `rm -rf apps/web/.next`, free :3000, restart). **⚠️ GOTCHA: PowerShell `Set-Content`/`Get-Content` CORRUPTS UTF-8 box-drawing/em-dash chars in source files (PS 5.1 reads as ANSI → mojibake `â”€`). For line-range deletions use Git-Bash `sed -i 'A,Bd'` (byte-safe) or the Edit tool — NEVER round-trip a source file through PS Get/Set-Content.** **PREVIEW-MCP serverId rotates each session + can be lost mid-session → free :3000 (`Stop-Process -Id <pid>`) + `preview_start web` to re-acquire (browser login persists).**

**▶▶ FOUNDER BACKLOG (2026-06-27) — a multi-epic stream-of-consciousness request. Building in order, each verified live + committed. Founder chose this order:**
- ✅ **EPIC 1 — Safari header layouts (DONE, `03258f9`).** classic/centered/split/minimal now work on the Safari header (`navigation.header.layout` → `buildSafariNav`→`SafariShell`→`SafariNav` + scoped `.lay-*` CSS, DOM order constant). Header nav-manager is true WYSIWYG (real `SafariShell`+hero) and the 4-card picker drives preview + live. See CHANGELOG.
- ⏳ **EPIC 2 — Menu manager overhaul** (founder: "spend a lot of time"): make it work smoothly in the UI, easy to control, **sub-menu item styling + layout** control. (`MenuStudio` + `NavSectionEditor` `[section]=menu`.)
- ⏳ **EPIC 3 — Forms: data & theme.** Room-select pre-fills the host's REAL rooms; **consent field** links to T&Cs + opts the guest into the host's marketing (sets marketing-subscribe on the guest record); forms auto-adopt the active theme's style (Safari). (`FormSection`/`forms.schema.ts`; `field.type==="rooms"` already exists — make it auto-populate.)
- ⏳ **EPIC 4 — Forms → Inbox.** Booking/enquiry creates an inbox **quote request** (not just an enquiry); every submission opens a host↔guest message **thread**; the website enquiry must be **formatted like a "Request a quote"** with a **"Website enquiry" pill**. (`createWebsiteEnquiry.ts` already opens a website-source conversation + system message — align its formatting with the quote-request inbox; wire booking enquiries to the quote pipeline.)

**ALSO FIXED THIS SESSION (not an epic):** phantom "Unpublished changes" indicator (`bc47720` — `computeWebsiteDirty`/`buildWebsiteSnapshot` now canonicalise navigation through the schema on both sides) + removed stray `loop test`/`loop two` test enquiries from the fixture inbox.
