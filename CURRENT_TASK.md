...
**▶ NEXT — Phase 4 (Form Builder) build slices 3–5. Decisions locked:** submissions → **inbox + the new submissions table** (reuse `lib/website/createWebsiteEnquiry` for email-bearing forms; persist every submission to `website_form_submissions`, store `conversation_id`); spam → **honeypot only for now** (Turnstile deferred, no env keys); newsletter → **CRM contacts** (`host_contacts` tag `newsletter` + `email_consent` via `upsertHostContact`).
