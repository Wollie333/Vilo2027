...
## ▶▶ SAVE POINT — RESUME HERE (· 2026-06-27 PM — header builder + menu builder DONE; ★ standard written; NEXT = FORMS epics)

**Branch:** `main` — working tree CLEAN, all work committed + **PUSHED** (latest `124042f`, origin == local). tsc + lint + **131 vitest** GREEN. **Verify with `cd apps/web && pnpm exec tsc --noEmit` + `pnpm next lint` + the Preview MCP — NEVER `pnpm build` while a dev server runs** (shared `.next`).
...
**▶▶ NEXT — FORMS epics (founder backlog, in service of the north star — forms are shared layer-2 machinery every theme inherits):**
