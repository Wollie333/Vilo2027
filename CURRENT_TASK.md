...
**▶ NEXT — Phase 5 (Minutes-to-Launch / AI Site Generator).** Contract: `WEBSITE_CMS_PLAN.md` §"Phase 5". Save-points: (a) `buildSiteBrief()` + generation engine (edge fn, Claude structured output where the tool schema = `WebsiteSection[]` + SEO + nav, Zod-validate+repair, draft-only, real photos + alt only); (b) onboarding wizard (stepper basics→vibe/theme→voice→pages→generate) + review/per-section regenerate; (c) inline assist (rewrite-section + SEO-suggest, `translate` stub) + guided first-run tour + guardrails (gating, token budget, optional `website_ai_generations` log). Start a fresh session; read this anchor + `WEBSITE_CMS_PLAN.md` §Phase 5 first.
