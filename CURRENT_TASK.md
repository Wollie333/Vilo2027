...
**⚠️ DELIBERATELY EXCLUDED FROM THE PROD PUSH: `supabase/migrations/20260628000004_seed_external_reviews_test_data.sql`** — it seeds MOCK Google/Facebook review sources (with `is_active=true` + fake tokens) onto the FIRST real host/property; on prod the daily cron would then try to sync fake sources. It's a dev-only convenience. **Left UNTRACKED on disk (not committed).** DECIDE: keep it as a local-only dev seed (move out of `migrations/` so it can't be applied to prod), or delete it.
