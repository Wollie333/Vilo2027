...
1. (lower) in-builder **Page settings** panel (SEO + a11y → retire the old dashboard `pages/[pageId]` route, still present as an unlinked fallback).
2. Fold header/footer/menu editors onto the page-builder engine (replace the bespoke `NavSectionEditor`).
3. (optional polish) container-query device preview for per-block responsive style; more free elements / column block kinds if the founder wants them.
