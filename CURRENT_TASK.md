...
**▶ Phase 6A — SLICE 3 (NEXT): pop-ups.** Extend `settings.conversion` (or sibling `settings.popup`) with a pop-up modal: trigger rules (delay / exit-intent / scroll-%) + frequency cap via localStorage, optional embedded newsletter reusing a `website_forms` form (the `form` render path already exists — `FormSection`/`loadSiteForms`/`submitWebsiteForm`). Must flow into `published_snapshot` (same freeze pattern as slice 2) + inject in `SiteChrome` (client component, like `AnnouncementBar`). Builder: Settings tab UI + save action.
