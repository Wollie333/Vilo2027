...
## ▶▶ SAVE POINT — RESUME HERE (· 2026-06-26, fresh-session checkpoint)

**Branch:** `main` — working tree CLEAN, **all work committed + PUSHED** (origin/main = local). tsc + lint GREEN (lint: only 2 pre-existing `<img>` warnings in `reports/_components`, untouched). **Verify with `cd apps/web && pnpm exec tsc --noEmit` + `pnpm next lint` + the Preview MCP — NEVER `pnpm build` while a dev server runs** (shared `.next`). A preview-managed dev server may still be on :3000 from last session (preview name `web`); a fresh session can reuse or restart it.
...
**▶▶ NEXT (start here in the fresh session) — the OTHER 6 SAFARI PAGES, same treatment as home:**
