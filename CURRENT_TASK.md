...
## ▶▶ SAVE POINT — RESUME HERE (· 2026-06-27 PM #6 — NAV CANVAS RESIZES PER SCREEN SIZE FOR **GENERIC THEMES TOO** — TASK #30 done)

**Branch:** `main` — working tree CLEAN, all work committed + **PUSHED** (origin == local). tsc + **131 vitest** GREEN. **Verify with `cd apps/web && pnpm exec tsc --noEmit` + `pnpm next lint` + the Preview MCP — NEVER `pnpm build` while a dev server runs** (shared `.next`).
