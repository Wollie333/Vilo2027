...
**LATEST (2026-06-25) — fix: room pages 404'd in the preview tab.** Room links dropped `preview=1` → room route loaded non-preview → 404 for unpublished/draft. Fix in `loadSitePage.ts`: `siteRoomHref` now adds `&preview=1` (+`?site=`) when `ctx.preview` (for room CARDS, which bypass buildNavHref); `roomMenuLinks` now returns a CLEAN `/rooms/<slug>` (header MenuLink's `buildNavHref` adds /site+site+preview itself — baking it too double-encoded the menu URL). Verified in preview: menu + card room links render the room-detail template (no 404). tsc+lint+181 vitest green.
