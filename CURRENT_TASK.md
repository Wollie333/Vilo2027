...
1. **Free elements — slice 2:** a **Columns** container block (bounded SINGLE-LEVEL: a section whose props carry N columns, each holding a small list of inline elements — NOT a general element tree) + **per-block desktop/tablet/mobile style overrides** (additive `sectionBase` field, e.g. `style?`, applied in `SectionRenderer`'s `SectionWrap` + an inspector control with the device toggle the builder already has).
