...
/**
 * The Marmalade House search-results page (the design's `.availbar` + `.sr-list` /
 * `.sr-card`). A self-contained search (dates + guests, pre-filled from the URL)
 * that quotes EVERY bookable property in parallel via /api/website-quote (price
 * recalculated server-side) and lists the available matches. In the builder
 * preview (`interactive=false`) it renders but never calls the endpoint. Scoped
 * to `.wielo-marmalade`.
 */
export function MarmaladeSearchResults({
  data,
  interactive = false,
}: {
      ...
      <div className="wrap">
        ...
        {error ? (
          ...
          <div className="sr-empty">
            ...
            <h2 className="lg">No availability for those dates</h2>
            ...
            {available.map((m, i) => (
              <article key={m.property.id} className="sr-card">
                ...
                <div className="sr-b">
                  ...
                  <p className="muted">
                    Final price confirmed at checkout — book direct for the best
                    rate.
                  </p>
                </div>
                <div className="sr-p">
