...
/** Marmalade House reuses the theme-agnostic nav link model from buildSafariNav. */
export type MarmaladeNavLink = SafariNavLink;
...
/** Host menu-style CSS scoped to the Marmalade House header + drawer. The scrolled
...
function menuStyleCss(style?: SiteMenuStyle | null): string {
  ...
  if (type.length)
    rules.push(
      `.wielo-marmalade .nav-links a,.wielo-marmalade .mnav-links a{${type.join(";")}}`,
    );
  if (color)
    rules.push(
      `.wielo-marmalade .nav-links a,.wielo-marmalade .mnav-links a{color:${color};opacity:1}`,
    );
  if (hover)
    rules.push(
      `.wielo-marmalade .nav-links a:hover,.wielo-marmalade .mnav-links a:hover{color:${hover};opacity:1}`,
    );
  const scrolledColor = style.scrolledColor?.trim();
  const scrolledHover = style.scrolledHoverColor?.trim() || scrolledColor;
  if (scrolledColor)
    rules.push(
      `.wielo-marmalade .nav.solid .nav-links a{color:${scrolledColor};opacity:1}`,
    );
  if (scrolledHover)
    rules.push(
      `.wielo-marmalade .nav.solid .nav-links a:hover{color:${scrolledHover};opacity:1}`,
    );
  if (typeof style.itemGap === "number")
    rules.push(`.wielo-marmalade .nav-links{gap:${style.itemGap}px}`);
  ...
  if (subBg)
    rules.push(
      `.wielo-marmalade .nav-sub{background:${subBg};border-color:${subBg}}`,
    );
  if (subColor)
    rules.push(
      `.wielo-marmalade .nav-sub a,.wielo-marmalade .mnav-sub a{color:${subColor}}`,
    );
  if (subHover)
    rules.push(
      `.wielo-marmalade .nav-sub a:hover,.wielo-marmalade .mnav-sub a:hover{color:${subHover}}`,
    );
  if (typeof style.fontSize === "number")
    rules.push(`.wielo-marmalade .nav-links a{font-size:${style.fontSize}px}`);
  ...
  if (tb) {
    ...
    const parts: string[] = [];
    if (tr.length) parts.push(`.wielo-marmalade .nav-links a{${tr.join(";")}}`);
    if (tb.hoverColor?.trim())
      parts.push(
        `.wielo-marmalade .nav-links a:hover{color:${tb.hoverColor.trim()}}`,
      );
    if (parts.length)
  ...
  if (mb) {
    if (mb.overlayBg?.trim())
      rules.push(`.wielo-marmalade .mnav{background:${mb.overlayBg.trim()}}`);
    const mr: string[] = [];
    ...
    if (mr.length)
      rules.push(`.wielo-marmalade .mnav-links a{${mr.join(";")}}`);
    if (mb.hoverColor?.trim())
      rules.push(
        `.wielo-marmalade .mnav-links a:hover{color:${mb.hoverColor.trim()}}`,
      );
  }
  return rules.join("");
}
...
function menuItemStyleCss(
  links: MarmaladeNavLink[],
  previewDevice?: "desktop" | "tablet" | "phone",
): string {
  const rules: string[] = [];
  const walk = (list: MarmaladeNavLink[]) => {
    for (const l of list) {
      const st = l.style;
      if (l.id && st) {
        const id = l.id;
        const inlineSel = `.wielo-marmalade .nav-links a.mi-${id},.wielo-marmalade .nav-sub a.mi-${id}`;
        const drawerSel = `.wielo-marmalade .mnav-links a.mi-${id},.wielo-marmalade .mnav-sub a.mi-${id},.wielo-marmalade .mnav-parent.mi-${id}`;
        ...
        if (inlineHover)
          rules.push(
            `.wielo-marmalade .nav-links a.mi-${id}:hover,.wielo-marmalade .nav-sub a.mi-${id}:hover{color:${inlineHover}}`,
          );
        ...
        if (drawerHover)
          rules.push(
            `.wielo-marmalade .mnav-links a.mi-${id}:hover,.wielo-marmalade .mnav-sub a.mi-${id}:hover{color:${drawerHover}}`,
          );
      }
...
/**
 * Marmalade House fixed header: transparent + white over the hero (`.nav.float.over`),
...
export function MarmaladeNav({
  ...
  const bookCss = bookColor?.trim()
    ? `.wielo-marmalade .nav-book-custom,.wielo-marmalade .nav-book-custom:hover{background:${bookColor.trim()}!important;border-color:${bookColor.trim()}!important;color:#fff!important}`
    : "";
  const styleCss =
  ...
  const logoRdCss =
    hasLogoOverride && !previewDevice
      ? ".wielo-marmalade .brand-rd-tb,.wielo-marmalade .brand-rd-mb{display:none}" +
        ".wielo-marmalade .brand-rd-dt{display:contents}" +
        "@media (max-width:1024px){.wielo-marmalade .brand-rd-dt{display:none}.wielo-marmalade .brand-rd-tb{display:contents}}" +
        "@media (max-width:640px){.wielo-marmalade .brand-rd-tb{display:none}.wielo-marmalade .brand-rd-mb{display:contents}}"
      : "";
