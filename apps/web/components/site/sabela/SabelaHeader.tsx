...
/** Brand lock-up: the host's logo when set, else a monogram in the gold mark,
 *  plus the serif wordmark (with an optional short locality/tagline subtitle). */
function Brand({
  brand,
  subtitle,
  href,
...
/**
 * Sabela Lodge bespoke HEADER — the founder's reference nav ported class-by-class
 * (scoped `.sbchrome`). A dark-first, editorial safari-lodge bar: a fixed header
 * that floats transparent over the full-bleed hero (light text) and fades to a
 * solid, blurred ebony surface on scroll (warm-bone ink); a centred primary menu
 * with CSS hover/focus dropdowns and a gold underline active state; a gold "Book
 * a stay" CTA; and a full-screen mobile drawer behind the burger. Wired to the
 * site's live brand + menu. Client-only for the scroll + drawer.
 */
export function SabelaHeader({
  brand,
  menu,
  bookHref,
  bookLabel,
  ...
  return (
    <div className="sbchrome">
      ...
      {!transparent ? <div aria-hidden style={{ height: 78 }} /> : null}
        ...
        <div className="wrap">
          <div className="nav-in">
                  ...
                  return (
                    <div key={item.id} className="has-sub">
                      <button
                        type="button"
                        className={
                          active ? "nav-link caret active" : "nav-link caret"
                        }
                      >
                        {item.label}
                        {ChevronIcon}
            ...
            <div className="nav-right">
              {bookHref ? (
                <a href={bookHref} className="btn btn-primary btn-sm">
                  {cta}
                </a>
              ) : null}
              <button
      ...
      <div className={open ? "mnav open" : "mnav"} aria-hidden={!open}>
        ...
        {bookHref ? (
          <a href={bookHref} className="btn btn-primary btn-lg btn-block">
            {cta}
          </a>
        ) : null}
      </div>
