...
// One formal contact-detail row — a champagne-ringed icon, a small uppercase
// label, and the value set in the Archivo display face, separated by a hairline
// rule. Rendered only when it carries a real value.
function DetailRow({
  icon,
  label,
  value,
  ...
  return (
    <div className="rc-drow">
      <span className="rc-dic" aria-hidden>
        {icon}
      </span>
      <div className="rc-dbody">
        <span className="rc-dlabel">{label}</span>
        <span className="rc-dval">{value}</span>
        {sub ? <span className="rc-dsub">{sub}</span> : null}
      </div>
    </div>
  );
}
...
/**
 * Royal Hotel CONTACT page (preset `royal`) — its own component + stylesheet
 * (`.rcontact` / royalContact.css): the formal grand-hotel treatment (Archivo
 * display, a centred champagne-ruled page head, hairline-ruled contact details,
 * the shared enquiry form inside a refined framed card) — distinct from the
 * OceansView resort Contact. The enquiry form is the SAME shared lead-capture
 * component (posts to the host inbox); the details column is wired to LIVE
 * establishment data (phone / email / address, plus the map embed when we can
 * build one). The FAQ follows the standard content flow: the host's wizard
 * `content_profile.contact.faq`, falling back to the property's real policies
 * (never fabricated prose); it omits entirely when there's nothing real to show.
 * Renders inside the shared themed chrome. Phase C (theme differentiation).
 */
export function RoyalContact({
  brandName,
  websiteId,
  interactive = false,
  heroImageUrl,
  ...
  return (
    <div className="rcontact">
      ...
      {/* DETAILS + FORM — formal, centred head */}
      <section className="section">
        <div className="wrap">
          ...
          <div className="rc-grid">
            ...
            <div className="rc-info" data-reveal>
              {hasDetails ? (
              ...
              {/* A real guest review — social proof beside the enquiry form.
                  Omitted when the site has no reviews. */}
              {review?.body?.trim() ? (
                <figure className="rc-quote">
                  <div
                    className="rc-qstars"
                  ...
                  <figcaption>
                    <span className="rc-qav">{initials(review.author)}</span>
                    <span>
                      <span className="rc-qnm">{review.author}</span>
                      {review.date ? (
                        <span className="rc-qdt">{review.date}</span>
                      ) : null}
                    </span>
                  </figcaption>
                </figure>
            ...
            {/* RIGHT — the shared enquiry form in a refined framed card */}
            <div className="rc-form" data-reveal>
              <div className="rc-formcard">
                <span className="tag">Send a message</span>
                <h3 className="rc-formtitle">Tell us about your trip</h3>
                <OceansContactForm
                  websiteId={websiteId}
      ...
      {mapEmbedUrl ? (
        ...
        <section className="section-sm" style={{ paddingTop: 0 }}>
          <div className="wrap">
            <div className="rc-mapph">
              <span className="rc-mappin" />
              <div className="rc-maptag">📍 {address}</div>
            </div>
          </div>
        </section>
      ) : null}
