...
/**
 * Oceans View SPECIAL/offer-detail page — the founder's bespoke reference design,
 * adapted from the room-detail template to a single offer: breadcrumb → rounded
 * hero (coral badge overlay) → title + prose + an offer-facts grid, a sticky
 * booking card (price/was/save + "Book this offer" CTA), and a "more offers"
 * strip. Omit-rather-than-fabricate: only facts that exist render. Server-rendered;
 * renders INSIDE the themed chrome (nav/footer from SiteChrome). Scoped `.ovsd`.
 */
export function OceansViewSpecialDetail({
  special,
  otherSpecials,
  specialsHref,
  asset,
