...
import { OceansViewNav, type OceansViewNavLink } from "./OceansViewNav";

import "./oceansview.css";

// Shared Oceans View chrome: the scoped `.wielo-oceansview` root, theme fonts,
// the scroll-aware nav (transparent white over the hero → frosted solid on
// scroll) and the navy footer. Every Oceans View page renders its content inside
...
export function OceansViewShell({
  brandName,
  nav,
  bookHref,
  solidNav = false,
  ...
  const monogram = (brandName.trim()[0] || "O").toUpperCase();
  const roomsHref =
  ...
  const blurb =
    foot.blurb?.trim() ||
    "A bright beachfront resort where the Atlantic starts at your door. Sea-view rooms, three pools, a spa, and a slower way to be by the sea.";
  ...
  const rootCls = ["wielo-oceansview", bar ? "pre" : ""]
    .filter(Boolean)
    .join(" ");
  ...
  return (
    ...
    >
      {bar ? (
        <SitePreviewBar themeName="Oceans View" pages={previewPages} />
      ) : null}
      {showTopBar ? (
      ...
      {/* Theme-scoped fonts (Bricolage Grotesque display + Manrope body). */}
      ...
      <ChromeEditWrap editable={editable} target="header" label="Header">
        <OceansViewNav
          brandName={brandName}
          monogram={monogram}
          tagline={nav.tagline?.trim() || "Beach Resort"}
