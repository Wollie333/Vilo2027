...
describe("classifyHost — app-domain routing is unchanged", () => {
  ...
  it("keeps the app domains on the app branch", () => {
    for (const h of [
      ...
      "127.0.0.1",
      "0.0.0.0",
      "wielo-git-main-team.vercel.app",
  ...
  it("treats reserved + locale subdomains as app (never a tenant)", () => {
    for (const sub of [
      ...
      expect(classifyHost(`${sub}.wielo.site`, ROOT)).toEqual({ kind: "app" });
    }
  });
  ...
  it("treats multi-level subdomains as app", () => {
    expect(classifyHost("a.b.wielo.site", ROOT).kind).toBe("app");
  });
...
describe("classifyHost — tenant routing", () => {
  it("routes tenant subdomains, normalising case + port", () => {
    expect(classifyHost("stillwater.wielo.site", ROOT)).toEqual({
      kind: "site",
      ref: "stillwater",
    });
    expect(classifyHost("Stillwater.Wielo.Site:443", ROOT)).toEqual({
      kind: "site",
      ref: "stillwater",
    });
  });
