...
export async function createSiteSpecialBooking(
  ...
  // Availability — the authoritative gate. Must EXCLUDE this special's own
  // calendar hold: an active fixed-date special blocks its dates
  // (blocked_dates.source='special', special_id=<this special>) to reserve them,
  // so the generic availability RPCs (which count every block) would wrongly
  // report the special's own dates as unavailable and make it unbookable on the
  // host's own website — refusing 100% of guests. Mirror the Wielo deal-page path
  // (deal/[slug]/book/actions.ts): query blocked_dates directly and ignore rows
  // belonging to THIS special; every other block (real bookings, manual holds,
  // iCal, other specials) still counts.
  {
    let q = admin
      .from("blocked_dates")
