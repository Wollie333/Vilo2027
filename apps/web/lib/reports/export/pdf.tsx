...
export interface ReportData {
  ...
  deep?: {
    ...
    avgPartySize: number;
    lengthOfStay: { label: string; count: number }[];
    leadTime: { label: string; count: number }[];
    partySize: { label: string; count: number }[];
    revenueByMonth: {
      month: string;
      revenue: number;
      adr: number;
  ...
  return (
    <Document>
      ...
      {data.deep ? (
        <Page size="A4" style={styles.page}>
          <View style={styles.header}>
            <Text style={styles.title}>In-depth analytics</Text>
            <Text style={styles.subtitle}>
              {data.hostName} · {data.startDate} to {data.endDate}
            </Text>
          </View>

          <Text style={styles.sectionTitle}>Booking behaviour</Text>
          <View style={styles.kpiRow}>
            <Kpi
              label="Avg length of stay"
              value={`${data.deep.avgNights} nights`}
            />
            <Kpi
              label="Median lead time"
              value={`${data.deep.medianLeadDays} days`}
            />
            <Kpi label="Avg party size" value={`${data.deep.avgPartySize}`} />
            <Kpi
              label="Repeat guest rate"
              value={`${data.deep.guests.repeatRate}%`}
            />
          </View>

          <Text style={styles.sectionTitle}>Length of stay (bookings)</Text>
          {data.deep.lengthOfStay.map((b) => (
            <View key={b.label} style={styles.listRow}>
              <Text>{b.label} nights</Text>
              <Text>{b.count}</Text>
            </View>
          ))}

          <Text style={styles.sectionTitle}>Booking lead time (bookings)</Text>
          {data.deep.leadTime.map((b) => (
            <View key={b.label} style={styles.listRow}>
              <Text>{b.label}</Text>
              <Text>{b.count}</Text>
            </View>
          ...
          {data.deep.revenueByMonth.length > 0 ? (
            <>
              <Text style={styles.sectionTitle}>Revenue & ADR by month</Text>
              <View style={styles.tableHeader}>
                <Text style={{ flex: 2 }}>Month</Text>
                <Text style={{ flex: 1, textAlign: "right" }}>Bookings</Text>
                <Text style={{ flex: 1, textAlign: "right" }}>Revenue</Text>
                <Text style={{ flex: 1, textAlign: "right" }}>ADR</Text>
              </View>
              {data.deep.revenueByMonth.map((m) => (
                <View key={m.month} style={styles.listRow}>
                  <Text style={{ flex: 2 }}>{m.month}</Text>
                  <Text style={{ flex: 1, textAlign: "right" }}>
                    {m.bookings}
                  </Text>
                  <Text style={{ flex: 1, textAlign: "right" }}>
                    R {formatNumber(m.revenue)}
                  </Text>
                  <Text style={{ flex: 1, textAlign: "right" }}>
                    R {formatNumber(m.adr)}
                  </Text>
                </View>
              ))}
            </>
          ...
          {data.deep.guests.top.length > 0 ? (
            <>
              <Text style={styles.sectionTitle}>
                Top guests by revenue ({data.deep.guests.returning} returning of{" "}
                {data.deep.guests.total})
              </Text>
              {data.deep.guests.top.map((g, i) => (
                <View key={i} style={styles.listRow}>
                  <Text>
                    {g.name} · {g.bookings} stays
                  </Text>
                  <Text>R {formatNumber(g.revenue)}</Text>
                </View>
              ))}
            </>
          ) : null}
          ...
          <Text style={styles.sectionTitle}>
            Cancellations — {data.deep.cancellations.total} (
            {data.deep.cancellations.rate}%)
          </Text>
          {data.deep.cancellations.reasons.length > 0 ? (
            data.deep.cancellations.reasons.map((r) => (
