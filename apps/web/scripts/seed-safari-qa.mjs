// Safari QA: re-point the test-site website (from seed-test-site.mjs) onto the
// SAFARI theme with the FULL standard page set, and add specials + add-ons so the
...
//
//   node --env-file=.env.local scripts/seed-safari-qa.mjs   # from apps/web
//
...
if (!URL || !SERVICE_KEY) {
  console.error("Missing env. Run: node --env-file=.env.local scripts/seed-safari-qa.mjs");
  process.exit(1);
}
...
// Default spines for the 4 standard pages Safari's blueprint doesn't ship.
...
async function main() {
  console.log(`Safari QA seed → ${URL}`);
  ...
  // Safari theme (base + blueprint)
  const { data: theme } = await admin
    .from("site_themes").select("slug, base, page_templates").eq("slug", "safari").maybeSingle();
  if (!theme) throw new Error("Safari theme not found in site_themes.");
  const blueprint = Array.isArray(theme.page_templates) ? theme.page_templates : [];

  // Point the website at Safari
  await admin.from("host_websites").update({
    status: "published",
    published_at: nowIso(),
    theme: theme.base ? { preset: "safari", base: theme.base } : { preset: "safari" },
    published_snapshot: null, // force the renderer to read live columns
  }).eq("id", WEBSITE_ID);

  // Rebuild pages: Safari blueprint + the 4 standard pages it omits.
  ...
  // Only add a standard page if the theme blueprint doesn't already ship it
  // (themes now seed their own designed Specials/Experiences/Gallery pages).
  ...
  console.log(`   seeded ${rows.length} pages on Safari`);
