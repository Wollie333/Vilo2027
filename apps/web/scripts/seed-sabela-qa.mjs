// Sabela QA: re-point the test-site website (from seed-test-site.mjs) onto the
// SABELA theme with the FULL standard page set, and add specials + add-ons so the
...
//
//   node --env-file=.env.local scripts/seed-sabela-qa.mjs   # from apps/web
//
...
if (!URL || !SERVICE_KEY) {
  console.error("Missing env. Run: node --env-file=.env.local scripts/seed-sabela-qa.mjs");
  process.exit(1);
}
...
// Default spines for the 4 standard pages Sabela's blueprint doesn't ship.
...
async function main() {
  console.log(`Sabela QA seed → ${URL}`);
  ...
  // Sabela theme (base + blueprint)
  const { data: theme } = await admin
    .from("site_themes").select("slug, base, page_templates").eq("slug", "sabela").maybeSingle();
  if (!theme) throw new Error("Sabela theme not found in site_themes.");
  const blueprint = Array.isArray(theme.page_templates) ? theme.page_templates : [];

  // Point the website at Sabela
  await admin.from("host_websites").update({
    status: "published",
    published_at: nowIso(),
    theme: theme.base ? { preset: "sabela", base: theme.base } : { preset: "sabela" },
    published_snapshot: null, // force the renderer to read live columns
  }).eq("id", WEBSITE_ID);

  // Rebuild pages: Sabela blueprint + the 4 standard pages it omits.
  ...
  console.log(`   seeded ${rows.length} pages on Sabela`);
