...
export async function finalizeOnboardingAction(
  ...
  // handle is auto-generated by trigger_host_handle from display_name.
  const hostInsert: Omit<
    Database["public"]["Tables"]["hosts"]["Insert"],
    "handle"
  > = {
    user_id: user.id,
  ...
  const { data: host, error: hostErr } = await admin
    .from("hosts")
    .insert(hostInsert as Database["public"]["Tables"]["hosts"]["Insert"])
    .select("id, handle")
    .single();
  if (hostErr || !host) {
    return {
  ...
  const { error: listingErr } = await admin
    .from("properties")
    .insert(
      listingInsert as Database["public"]["Tables"]["properties"]["Insert"],
    );
  if (listingErr) {
