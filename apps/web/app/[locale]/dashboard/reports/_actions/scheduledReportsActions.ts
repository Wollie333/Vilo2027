...
export async function updateScheduledReportAction(
  ...
  if (input.report_type !== undefined)
    updateData.report_type = input.report_type;
  ...
  if (input.scope_filter !== undefined)
    updateData.scope_filter = input.scope_filter;
 ...
 */
export async function toggleScheduledReportActiveAction(
  reportId: string,
  isActive: boolean,
) {
  const supabase = createServerClient();
