    ...
    <div className="rounded-card border border-brand-line bg-white p-5 lg:p-6">
      ...
      <div className="space-y-3">
        {data.map((region) => {
          ...
          const color =
            PROVINCE_COLORS[region.province] || PROVINCE_COLORS["Unknown"];
              ...
              <div className="mb-1 flex items-center justify-between text-xs">
                ...
                <div className="flex items-center gap-3">
                  <span className="text-brand-mute">
                    {region.bookings}{" "}
                    {region.bookings === 1 ? "booking" : "bookings"}
                  </span>
      ...
      <div className="mt-5 rounded border border-brand-accent bg-brand-accent/10 p-3">
        <div className="text-xs text-brand-ink">
          ...
          {data[0]?.province} accounts for {data[0]?.percentage.toFixed(1)}% of
          total revenue (R {formatCurrency(data[0]?.revenue)})
        </div>
      </div>
    </div>
  );
