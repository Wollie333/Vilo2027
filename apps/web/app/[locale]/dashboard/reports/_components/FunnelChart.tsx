    ...
    <div className="rounded-card border border-brand-line bg-white p-5 lg:p-6">
      ...
      <div className="mb-5">
        ...
        <h3 className="mt-1 font-display text-lg font-bold text-brand-ink">
          {formatNumber(data.views)} views → {formatNumber(data.bookings)}{" "}
          bookings
        </h3>
        <div className="mt-0.5 text-xs text-brand-mute">
          Overall conversion:{" "}
          {(conversionRates.views_to_bookings ?? 0).toFixed(1)}% · Track guest
          journey from discovery to booking
        </div>
      </div>
            ...
            <div key={step.label}>
              ...
              {conversionRate !== null && conversionRate !== undefined && (
                <div className="ml-11 mt-2 flex items-center gap-1.5 text-xs text-brand-mute">
                  ...
                  <span>{conversionRate.toFixed(1)}% convert to next step</span>
                </div>
              )}
            </div>
          );
