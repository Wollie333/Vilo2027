...
import { addQuoteNoteAction } from "../actions";
...
export function QuoteInternalNotes({
  ...
  function submit() {
    ...
    start(async () => {
      const result = await addQuoteNoteAction(quoteId, text);
      if (result.ok) {
        setValue("");
        router.refresh();
    ...
    <div className="p-4">
      {notes.length > 0 ? (
        ...
        <p className="text-[12.5px] text-brand-mute">
          No notes yet. Anything you jot here stays host-only — the guest never
          sees it.
        </p>
      )}
      ...
      <div className="mt-3 flex items-center gap-2">
        <input
          ...
          disabled={pending}
          className="num w-full rounded-[10px] border border-brand-line px-3 py-2 text-[12.5px] text-brand-ink placeholder:text-brand-mute focus:border-brand-primary focus:outline-none focus:ring-4 focus:ring-brand-primary/10 disabled:opacity-60"
        />
        <button
          type="button"
          onClick={submit}
