...
import { removeBusinessLogoAction, uploadBusinessLogoAction } from "../actions";
...
export function BusinessLogoUploader({
  ...
  function pick(file: File) {
    ...
    start(async () => {
      ...
      const res = await uploadBusinessLogoAction(businessId, fd);
      if (!res.ok) {
  ...
  function remove() {
    start(async () => {
      const res = await removeBusinessLogoAction(businessId);
      if (!res.ok) {
        toast.error(res.error);
        return;
      }
