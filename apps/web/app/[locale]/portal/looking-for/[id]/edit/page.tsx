...
export default async function EditRequestPage({ params }: Props) {
  ...
  const initial: RequestEditValues = {
    ...
    childAges: (post.child_ages ?? []).map((a: number) => String(a)),
    pets: post.pets != null ? String(post.pets) : "",
    locationText: post.location_text ?? "",
    region: post.location_region ?? "",
    destinationFlexible: post.destination_flexible ?? false,
    locationLat: post.location_lat != null ? String(post.location_lat) : "",
    locationLng: post.location_lng != null ? String(post.location_lng) : "",
    searchRadiusKm:
      post.search_radius_km != null ? String(post.search_radius_km) : "",
