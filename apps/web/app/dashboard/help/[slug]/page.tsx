...
export async function generateMetadata({
  ...
  if (!article) return { title: "Article not found · Vilo" };
  return {
...
export default async function HelpArticlePage({ params }: { params: Params }) {
  ...
  if (!user) redirect(`/login?next=/dashboard/help/${params.slug}`);
      ...
      <article className="mt-4 grid gap-8 lg:grid-cols-[1fr_280px]">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2 text-[11px] text-brand-mute">
            {category ? (
              <Link
                href={`/dashboard/help/category/${(category as { slug: string }).slug}`}
            ...
            <span className="inline-flex items-center gap-1">
              <ClockIcon className="h-3 w-3" /> {article.read_time_minutes} min
              read
            </span>
                  ...
                  <li key={r.id}>
                    ...
                    >
                      ...
                      <div className="mt-0.5 inline-flex items-center gap-1 text-[11px] text-brand-mute">
                        <ClockIcon className="h-3 w-3" /> {r.read_time_minutes}{" "}
                        min read
                      </div>
                    </Link>
                  </li>
