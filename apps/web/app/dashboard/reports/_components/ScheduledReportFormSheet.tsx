  ...
  const {
    ...
    defaultValues: editingReport
      ? {
          ...
          report_type: editingReport.report_type as ScheduledReportFormData["report_type"],
  ...
  return (
    <Sheet open={open} onOpenChange={onClose}>
      <SheetContent className="w-full overflow-y-auto sm:max-w-lg">
        <SheetHeader>
          <SheetTitle>{editingReport ? "Edit" : "Create"} Scheduled Report</SheetTitle>
          <SheetDescription>
        ...
        <form onSubmit={handleSubmit(handleFormSubmit)} className="mt-6 space-y-6">
          ...
          {/* Report Type */}
          <div>
            <label htmlFor="report_type" className="mb-1.5 block text-sm font-medium text-brand-ink">
            ...
            >
              ...
              <option value="refunds_cancellations">Refunds & Cancellations</option>
              <option value="occupancy_forecast">Occupancy Forecast</option>
            </select>
            {errors.report_type && (
              <p className="mt-1 text-xs text-red-600">{errors.report_type.message}</p>
            )}
          </div>

          {/* Schedule */}
          <div>
            <label htmlFor="schedule_preset" className="mb-1.5 block text-sm font-medium text-brand-ink">
            ...
            <select
              ...
              onChange={(e) => {
                const preset = e.target.value as ScheduledReportFormData["schedule_preset"];
          ...
          {schedulePreset === "custom" && (
            <div>
              <label htmlFor="schedule_cron" className="mb-1.5 block text-sm font-medium text-brand-ink">
              ...
              <input
                ...
                placeholder="0 8 * * *"
              />
              <p className="mt-1 text-xs text-brand-mute">Format: minute hour day month weekday</p>
              {errors.schedule_cron && (
                <p className="mt-1 text-xs text-red-600">{errors.schedule_cron.message}</p>
              )}
            </div>
          )}
          ...
          {/* Format */}
          <div>
            <label htmlFor="format" className="mb-1.5 block text-sm font-medium text-brand-ink">
          ...
          <div>
            ...
            {recipients.length > 0 && (
              <div className="mb-3 space-y-2">
                {recipients.map((recipient, index) => (
                  ...
                  >
                    <div>
                      <div className="text-sm font-medium text-brand-ink">{recipient.name}</div>
                      <div className="text-xs text-brand-mute">{recipient.email}</div>
                    </div>
                    <button
                      type="button"
            ...
            {errors.recipients && (
              <p className="mt-1 text-xs text-red-600">{errors.recipients.message}</p>
            )}
          </div>
          ...
          <div className="flex items-center gap-3 border-t border-brand-line pt-6">
            ...
            >
              {isSubmitting ? "Saving..." : editingReport ? "Update" : "Create"} Report
            </button>
          </div>
        </form>
      </SheetContent>
