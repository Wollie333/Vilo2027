...
import { Clock, FileText, Mail, MoreVertical, Play, Pause, Edit, Trash2, Plus } from "lucide-react";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
  ...
  if (reports.length === 0) {
    return (
      <div className="rounded-card border border-brand-line bg-white p-8 text-center">
        ...
        <h3 className="mb-2 text-lg font-semibold text-brand-ink">No scheduled reports yet</h3>
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-semibold text-brand-ink">Scheduled Reports</h3>
        <button
          onClick={onCreate}
      ...
      <div className="overflow-hidden rounded-card border border-brand-line bg-white">
        <table className="w-full">
          ...
          <tbody className="divide-y divide-brand-line">
            {reports.map((report) => (
              <tr key={report.id} className="transition-colors hover:bg-brand-light/30">
                <td className="px-4 py-3">
                  <div className="flex items-start gap-3">
                    <FileText className="mt-0.5 h-5 w-5 text-brand-primary" />
                    <div>
                      <div className="font-medium text-brand-ink">{report.name}</div>
                      <div className="text-xs text-brand-mute">{formatReportType(report.report_type)}</div>
                    </div>
                  </div>
                </td>
                <td className="px-4 py-3">
                ...
                <td className="px-4 py-3 text-sm text-brand-mute">{formatDate(report.last_run_at)}</td>
                <td className="px-4 py-3 text-sm text-brand-ink">{formatDate(report.next_run_at)}</td>
                <td className="px-4 py-3">
                  {report.is_active ? (
