...
export function GuestDemographics({ data }: GuestDemographicsProps) {
  ...
  const guestTypeData = [
    { name: "Returning", value: data.returning_guests, color: GUEST_TYPE_COLORS.returning },
    { name: "New", value: data.new_guests, color: GUEST_TYPE_COLORS.new },
  ];

  const returningPercentage = ((data.returning_guests / totalGuests) * 100).toFixed(1);
                  ...
                  content={({ active, payload }) => {
                    ...
                    return (
                      <div className="rounded border border-brand-line bg-white px-3 py-2 shadow-lg">
                        <div className="text-xs font-semibold text-brand-ink">{data.name}</div>
                        <div className="text-xs text-brand-mute">
                          {data.value} guests ({((data.value / totalGuests) * 100).toFixed(1)}%)
                        </div>
                      </div>
                    );
                  }}
          ...
          <div className="flex-1 space-y-2">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <div className="h-3 w-3 rounded" style={{ backgroundColor: GUEST_TYPE_COLORS.returning }}></div>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <div className="h-3 w-3 rounded" style={{ backgroundColor: GUEST_TYPE_COLORS.new }}></div>
                <span className="text-xs text-brand-mute">New</span>
              </div>
              <div className="text-sm font-semibold text-brand-ink">
                {data.new_guests} ({((data.new_guests / totalGuests) * 100).toFixed(1)}%)
              </div>
            </div>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="space-y-2">
            {data.country_breakdown.map((country) => {
              const color = COUNTRY_COLORS[country.country] || COUNTRY_COLORS["Unknown"];
