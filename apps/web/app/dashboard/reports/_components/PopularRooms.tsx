            ...
            <div className="flex h-8 w-8 shrink-0 items-center justify-center">
              {index === 0 ? (
                ...
                <div className="flex h-8 w-8 items-center justify-center rounded-full bg-brand-light">
                  <span className="text-sm font-bold text-brand-mute">{index + 1}</span>
                </div>
              )}
            </div>
      ...
      {data.length > 0 && (
        <div className="mt-5 rounded border border-brand-accent bg-brand-accent/10 p-3">
          <div className="text-xs text-brand-ink">
            ...
            {data[0].listing_name} achieved {data[0].occupancy_rate.toFixed(1)}% occupancy
            with R {formatCurrency(data[0].revenue)} revenue
          </div>
        </div>
      )}
    </div>
