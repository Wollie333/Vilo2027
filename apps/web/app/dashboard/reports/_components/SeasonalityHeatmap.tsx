...
export function SeasonalityHeatmap({ data, year }: SeasonalityHeatmapProps) {
  if (!data || !data.data || data.data.length === 0 || data.provinces.length === 0) {
    return (
    ...
    <div className="rounded-card border border-brand-line bg-white p-5 lg:p-6">
      ...
      <div className="overflow-x-auto">
        <div className="min-w-[500px]">
          {/* Province Headers */}
          <div className="mb-2 grid gap-1" style={{ gridTemplateColumns: `80px repeat(${data.provinces.length}, 1fr)` }}>
            <div></div>
          ...
          <div className="space-y-1">
            {data.data.map((monthData) => (
              <div
                ...
                style={{ gridTemplateColumns: `80px repeat(${data.provinces.length}, 1fr)` }}
              >
                {/* Month Label */}
      ...
      <div className="mt-3 flex items-start gap-2 rounded border border-brand-accent bg-brand-accent/10 p-3">
        ...
        <div className="text-xs text-brand-ink">
          <strong className="font-semibold">Peak season:</strong> {peakMonth.month}{" "}
          generated R {formatCurrency(peakMonth.total)} across all regions
        </div>
      </div>
    </div>
  );
