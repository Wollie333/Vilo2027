// Pure discount maths shared by the booking sidebar (estimate) and the
// server booking action (source of truth) so the two can never drift.
// Non-client module → safe to import from both "use client" and server code.
