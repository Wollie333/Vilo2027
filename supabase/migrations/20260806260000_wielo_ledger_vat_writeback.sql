-- Write VAT back onto the platform_ledger charge row.
--
-- mint_wielo_invoice_on_ledger_complete() already computes the VAT correctly for
-- BOTH modes (it backs VAT out of the gross NEW.amount using the live wielo
-- profile) and stores it on wielo_invoices.vat_amount — but platform_ledger.
-- vat_amount was left NULL. That column is what the ledger read-model
-- (lib/billing/wielo-ledger.ts), the host/Wielo statements and the affiliate
-- commission RPC (amount - vat_amount = ex-VAT net) all read, so VAT never
-- showed on the ledger and commission was accrued on the gross.
--
-- Fix: have the SAME trigger set platform_ledger.vat_amount = v_vat on the row it
-- just invoiced — byte-for-byte the invoice's VAT, so the ledger and the invoice
-- can never diverge. The self-UPDATE re-fires this AFTER trigger, but the
-- "invoice already exists" guard returns early, so there is no re-mint or loop.
...
BEGIN
  ...
  -- Write the invoice link AND the VAT back onto the ledger row. vat_amount is
  -- exactly the invoice's v_vat, so ledger and invoice reconcile by construction,
  -- and the commission RPC (amount - vat_amount) accrues on the ex-VAT net in
  -- both modes. The re-fire of this AFTER trigger short-circuits on the
  -- "invoice already exists" guard above.
  UPDATE platform_ledger
    SET invoice_id = v_inv_id, vat_amount = v_vat
    WHERE id = NEW.id;
  RETURN NEW;
END;
$function$;
