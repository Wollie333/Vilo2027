-- Migration: let the host choose how long before check-in access details are sent
--
-- Until now the access card + stay-details email went out on a hardcoded 60-min
-- lead (send_due_access_cards() used `now() + interval '60 minutes'` as the
-- upper bound of its "due" window). Founder ask: make that lead configurable per
-- property, defaulting to 60 minutes.
--
-- Stored on property_access (the per-property Guest Access config) as
-- send_lead_minutes. The trip page's code-unlock gate reads the same value, so
-- what the guest is emailed and what unlocks on the trip page stay in lockstep.

ALTER TABLE public.property_access
  ADD COLUMN IF NOT EXISTS send_lead_minutes integer NOT NULL DEFAULT 60
    CHECK (send_lead_minutes BETWEEN 15 AND 10080); -- 15 min … 7 days

COMMENT ON COLUMN public.property_access.send_lead_minutes IS
  'How many minutes before check-in the access card + stay-details email are sent (and codes unlock on the trip page). Default 60.';

-- Re-point send_due_access_cards() at the per-property lead. Everything else is
-- identical to 20260712110000 (which added the stay_details_guest email enqueue).
...
BEGIN
  FOR b IN
    ...
    LEFT JOIN property_access pa ON pa.property_id = bk.property_id
    WHERE bk.status IN ('confirmed', 'checked_in')
      ...
      AND ((bk.check_in::text || ' ' || COALESCE(l.check_in_time, '00:00'))::timestamp
          ...
          BETWEEN now() - interval '12 hours'
              AND now() + (COALESCE(pa.send_lead_minutes, 60)::text || ' minutes')::interval
  LOOP
...
COMMENT ON FUNCTION public.send_due_access_cards() IS
  'Every 15 min: sends the inbox access card (host→guest) + the stay_details_guest email once per booking, at each property''s configured lead (property_access.send_lead_minutes, default 60) before check-in.';
