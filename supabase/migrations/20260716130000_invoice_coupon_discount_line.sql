-- Itemize the coupon discount on the booking invoice. ensure_booking_invoice
-- built line_items with no discount line, but the invoice renderer already reads
-- line_items.discount_amount (invoice PDF route + hosted page) -> the coupon
-- reduction was baked into the total but never shown. Add discount_amount +
-- coupon_code to line_items so the invoice document itemizes it.
