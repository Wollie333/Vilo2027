-- Fix app_purge_user_account() to match the LIVE schema (migration drift):
--   1. impersonation_sessions has no "impersonating" column — it's target_user_id.
--   2. refund_requests.payment_id RESTRICT-references payments, so refund rows
--      (refund_status_history → refunds → refund_requests) MUST be cleared
--      BEFORE payments, or the payments delete violates the FK.
--
-- Same contract as 20260531000021: hard-purge every public-schema RESTRICT
-- dependent of a user (as guest + as host) so auth.users→user_profiles can
-- cascade. CREATE OR REPLACE — also repairs the in-app delete-account flow,
-- which used this RPC and was failing on both bugs above.
...
BEGIN
  ...
  -- ── Refund graph FIRST (refund_requests.payment_id → payments RESTRICT) ──
  -- refund_status_history → refund_requests, refunds → payments/bookings.
  ...
  -- ── Now the rest of the booking children, then bookings ─────────────────
