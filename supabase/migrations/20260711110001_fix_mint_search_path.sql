-- Make the credit-note mint trigger self-contained: pin its search_path and
-- schema-qualify the counter call. Previously it relied on the caller's
-- search_path to resolve next_wielo_credit_note_number(); when the mint fires
-- from inside another SECURITY DEFINER trigger (the new affiliate-ledger emit),
-- that resolution failed ("function next_wielo_credit_note_number() does not
-- exist") and rolled the whole event back. Pinning search_path fixes it for
-- every call path (app inserts + trigger chains).
...
BEGIN
  ...
  INSERT INTO public.wielo_credit_notes (
    ...
    public.next_wielo_credit_note_number(), NEW.type, NEW.id, NEW.user_id,
