-- Region filter (p_region) on the host analytics RPCs.
-- Adds an optional p_region text param that scopes each query to bookings on
-- properties in that province, alongside the existing p_listing_id filter.
-- Old signatures are dropped first so the new (one-longer) signature is the
-- only overload (no ambiguity). Generated from pg_get_functiondef dumps.

DROP FUNCTION IF EXISTS public.fetch_primary_kpis(uuid, date, date, uuid, text);
CREATE OR REPLACE FUNCTION public.fetch_primary_kpis(p_host_id uuid, p_start_date date, p_end_date date, p_listing_id uuid DEFAULT NULL::uuid, p_channel text DEFAULT NULL::text, p_region text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  WHERE host_id = p_host_id
    ...
    AND (p_region IS NULL OR property_id IN (SELECT id FROM properties WHERE host_id = p_host_id AND province = p_region AND deleted_at IS NULL))
  ...
  FROM (
    ...
    LEFT JOIN bookings b ON
      ...
      AND (p_region IS NULL OR b.property_id IN (SELECT id FROM properties WHERE host_id = p_host_id AND province = p_region AND deleted_at IS NULL))
...
END;
$function$;
GRANT EXECUTE ON FUNCTION public.fetch_primary_kpis TO authenticated;

DROP FUNCTION IF EXISTS public.fetch_secondary_metrics(uuid, date, date, uuid, text);
CREATE OR REPLACE FUNCTION public.fetch_secondary_metrics(p_host_id uuid, p_start_date date, p_end_date date, p_listing_id uuid DEFAULT NULL::uuid, p_channel text DEFAULT NULL::text, p_region text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
BEGIN
  ...
  WHERE property_id IN (
    ...
    WHERE host_id = p_host_id AND deleted_at IS NULL
      ...
      AND (p_region IS NULL OR province = p_region)
  )
...
END;
$function$;
GRANT EXECUTE ON FUNCTION public.fetch_secondary_metrics TO authenticated;

DROP FUNCTION IF EXISTS public.fetch_revenue_trend(uuid, date, date, text, uuid, text);
CREATE OR REPLACE FUNCTION public.fetch_revenue_trend(p_host_id uuid, p_start_date date, p_end_date date, p_grouping text DEFAULT 'day'::text, p_listing_id uuid DEFAULT NULL::uuid, p_channel text DEFAULT NULL::text, p_region text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
END;
$function$;
GRANT EXECUTE ON FUNCTION public.fetch_revenue_trend TO authenticated;

DROP FUNCTION IF EXISTS public.fetch_channel_mix(uuid, date, date, uuid);
CREATE OR REPLACE FUNCTION public.fetch_channel_mix(p_host_id uuid, p_start_date date, p_end_date date, p_listing_id uuid DEFAULT NULL::uuid, p_region text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
END;
$function$;
GRANT EXECUTE ON FUNCTION public.fetch_channel_mix TO authenticated;

DROP FUNCTION IF EXISTS public.fetch_conversion_funnel(uuid, date, date, uuid);
CREATE OR REPLACE FUNCTION public.fetch_conversion_funnel(p_host_id uuid, p_start_date date, p_end_date date, p_listing_id uuid DEFAULT NULL::uuid, p_region text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
END;
$function$;
GRANT EXECUTE ON FUNCTION public.fetch_conversion_funnel TO authenticated;

DROP FUNCTION IF EXISTS public.fetch_time_to_book(uuid, date, date, uuid);
CREATE OR REPLACE FUNCTION public.fetch_time_to_book(p_host_id uuid, p_start_date date, p_end_date date, p_listing_id uuid DEFAULT NULL::uuid, p_region text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
END;
$function$;
GRANT EXECUTE ON FUNCTION public.fetch_time_to_book TO authenticated;

DROP FUNCTION IF EXISTS public.fetch_regional_breakdown(uuid, date, date, uuid);
CREATE OR REPLACE FUNCTION public.fetch_regional_breakdown(p_host_id uuid, p_start_date date, p_end_date date, p_listing_id uuid DEFAULT NULL::uuid, p_region text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
BEGIN
  ...
  FROM (
    ...
    WHERE l.host_id = p_host_id AND l.deleted_at IS NULL
      AND (p_region IS NULL OR l.province = p_region)
...
END;
$function$;
GRANT EXECUTE ON FUNCTION public.fetch_regional_breakdown TO authenticated;

DROP FUNCTION IF EXISTS public.fetch_guest_demographics(uuid, date, date, uuid);
CREATE OR REPLACE FUNCTION public.fetch_guest_demographics(p_host_id uuid, p_start_date date, p_end_date date, p_listing_id uuid DEFAULT NULL::uuid, p_region text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
END;
$function$;
GRANT EXECUTE ON FUNCTION public.fetch_guest_demographics TO authenticated;

DROP FUNCTION IF EXISTS public.fetch_popular_rooms(uuid, date, date, integer);
CREATE OR REPLACE FUNCTION public.fetch_popular_rooms(p_host_id uuid, p_start_date date, p_end_date date, p_limit integer DEFAULT 5, p_region text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
END;
$function$;
GRANT EXECUTE ON FUNCTION public.fetch_popular_rooms TO authenticated;

DROP FUNCTION IF EXISTS public.fetch_refunds_cancellations(uuid, date, date, uuid);
CREATE OR REPLACE FUNCTION public.fetch_refunds_cancellations(p_host_id uuid, p_start_date date, p_end_date date, p_listing_id uuid DEFAULT NULL::uuid, p_region text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
END;
$function$;
GRANT EXECUTE ON FUNCTION public.fetch_refunds_cancellations TO authenticated;

DROP FUNCTION IF EXISTS public.fetch_property_performance(uuid, date, date, text, text, integer, integer);
CREATE OR REPLACE FUNCTION public.fetch_property_performance(p_host_id uuid, p_start_date date, p_end_date date, p_sort_by text DEFAULT 'revenue'::text, p_sort_direction text DEFAULT 'desc'::text, p_limit integer DEFAULT 25, p_offset integer DEFAULT 0, p_region text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
END;
$function$;
GRANT EXECUTE ON FUNCTION public.fetch_property_performance TO authenticated;
