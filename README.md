...
>
> **Active initiative (Jul 2026): Builder × Theme pixel-perfect pipeline** — plan of record [`docs/features/BUILDER_THEME_PLAN.md`](./docs/features/BUILDER_THEME_PLAN.md). Progress: ✅ **Phase 0** — themed date-picker/search-field clipping fixed (calendar portals above all content) · ✅ **Phase 1** — theme preview renders pixel-perfect with **stock data** (auto-populate blocks show representative content regardless of host setup) · ✅ **Phase 2** — activation seeds all pages incl. system templates, each inherits the theme + is builder-editable (verified: live=real data, preview=stock, system pages themed 200) · ⏳ Phases 3–6 (required system-blocks · Wielo data modals · per-block style UI · setup wizard + go-live gate).
