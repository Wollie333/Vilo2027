...
>
> **Active initiative (Jul 2026): Builder × Theme pixel-perfect pipeline** — plan of record [`docs/features/BUILDER_THEME_PLAN.md`](./docs/features/BUILDER_THEME_PLAN.md). Progress: ✅ **Phase 0** — themed date-picker/search-field clipping fixed (calendar portals above all content) · ✅ **Phase 1** — theme preview renders pixel-perfect with **stock data** (auto-populate blocks show representative content regardless of host setup) · ✅ **Phase 2** — activation seeds all pages incl. system templates, each inherits the theme + is builder-editable · ✅ **Phase 3** — required Wielo blocks per system template (contract SSOT + library "Req" badges + delete guard + publish guard; verified live on room_detail) · ⏳ Phases 4–6 (Wielo data modals · per-block style UI · setup wizard + go-live gate).
