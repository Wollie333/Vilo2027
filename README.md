...
>
> **Active initiative (Jul 2026): Admin platform hardening — COMPLETE.** All **20 admin sidebar tabs** are feature-ready, deep-functionally tested live (every action performs → fires its side-effect → logs to the immutable audit log + per-user History), and re-verified in a full sweep (save points #45–#46). Highlights: a systemic audit-constraint reconcile so **no admin action can silently fail to audit**; two hard-delete bugs found + fixed to true soft-delete (never-hard-delete rule); the **affiliate programme** built end-to-end (per-product commissions + setup-fee, proportional clawback, admin min-payout floor, tiers, link builder + QR, per-affiliate funnel, leaderboard, monthly statement PDFs, and **full Wielo-ledger integration** — affiliate money is real `platform_ledger` rows that auto-mint numbered credit-note docs — plus in-app/push/email notifications firing from every runtime incl. Deno webhook renewals). Tracking doc: [`ADMIN_MVP_CHECKLIST.md`](./ADMIN_MVP_CHECKLIST.md). **Next:** pre-launch test-data wipe + founder ops config (VAT #, banking/SWIFT, `PAYMENT_CIPHER_KEY`, Turnstile, `paystack-webhook` redeploy).
...
_Honest, code-level snapshot — **2026-07-11**. Percentages are share of MVP scope **wired in code** (UI → Server Action/RPC → real DB), not "verified in production." Build priority: **Host dashboard → Website CMS → Guest portal → Admin**._
...
### Admin panel — ~98% (all 20 tabs feature-ready + hardened)
Two-layer RBAC (active `platform_staff` + granular permission keys), **every mutation audited** — a systemic constraint reconcile guarantees no admin action can silently fail to audit, and host-scoped actions stamp `owner_user_id` so they surface in the affected user's **History** timeline.
**Working (all 20 sidebar tabs deep-tested live — see [`ADMIN_MVP_CHECKLIST.md`](./ADMIN_MVP_CHECKLIST.md)):** **Overview** control-centre (MRR/ARR/ARPU/churn, needs-attention tiles, growth footprint, latest-actions feed); **user management** (uniform record tabs — Overview/Bookings/Listings/Website/Products/Finance/Business/Affiliate/Reviews/History; edit/role/suspend/**true soft-delete**/notes/subscription + admin password reset + support-access grant flow); **support inbox** (host↔Wielo threads); **listings** moderation; **products & plans** full CRUD + per-product feature permissions + **per-product affiliate commissions incl. a wired setup-fee** + per-host overrides; **Wielo revenue ledger** (record payment / refund / credit note / adjustment → signed ledger rows that **auto-mint numbered invoices + credit-notes** with bank details + VAT split; running per-user balance; CSV); read-only **payments** records; **affiliate programme** admin (per-affiliate funnel, payout queue, tier editor, processor fees, marketing manager, terms editor); **reporting** (charts + PDF export); **review moderation**; **POPIA/GDPR** access + erasure queue (erasure is **always anonymise**, never hard-delete); **platform settings** (branding, Wielo business + VAT, Paystack/PayPal/EFT with secrets encrypted at rest, legal, tracking); **feature flags** (per-host overrides + guest permissions); **categories / deal-categories / amenities** taxonomy CRUD (with live public-page propagation); **broadcasts**; **direct notification send**; **email templates** (28 registered, preview + test-send over the queue→cron→Resend pipeline); immutable **audit log** viewer.
