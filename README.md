...
>
> **Active initiative (Jul 2026): Builder × Theme pixel-perfect pipeline** — plan of record [`docs/features/BUILDER_THEME_PLAN.md`](./docs/features/BUILDER_THEME_PLAN.md). Progress: ✅ **Phase 0** — themed date-picker/search-field clipping fixed (calendar portals above all content) · ✅ **Phase 1** — theme preview renders pixel-perfect with **stock data** (auto-populate blocks show representative content regardless of host setup) · ✅ **Phase 2** — activation seeds all pages incl. system templates, each inherits the theme + is builder-editable · ✅ **Phase 3** — required Wielo blocks per system template (contract + library "Req" badges + delete/publish guards) · 🟡 **Phase 4a** — edit real room data from the builder ("Edit room data…" modal writes to `property_rooms` via the existing action; verified live: a price edit persisted to the DB) · ⏳ Phases 4b–6 (real data on canvas + more block families · per-block style UI · setup wizard + go-live gate).
